...
#define EN(x) using x = ::x; using enum x
...
constexpr const auto vk_api_version_1_2 = VK_API_VERSION_1_2;
...
EN(VkAccessFlagBits);
EN(VkAttachmentLoadOp);
EN(VkAttachmentStoreOp);
EN(VkBlendFactor);
EN(VkBorderColor);
EN(VkBufferUsageFlagBits);
EN(VkChromaLocation);
EN(VkColorComponentFlagBits);
EN(VkColorSpaceKHR);
EN(VkCommandBufferLevel);
EN(VkCommandBufferUsageFlagBits);
EN(VkCommandPoolCreateFlagBits);
EN(VkCompareOp);
EN(VkComponentSwizzle);
EN(VkCompositeAlphaFlagBitsKHR);
EN(VkCullModeFlagBits);
EN(VkDebugUtilsMessageSeverityFlagBitsEXT);
EN(VkDebugUtilsMessageTypeFlagBitsEXT);
EN(VkDependencyFlagBits);
EN(VkDescriptorType);
EN(VkDynamicState);
EN(VkFenceCreateFlagBits);
EN(VkFilter);
EN(VkFormat);
...
EN(VkImageTiling);
EN(VkImageType);
EN(VkImageUsageFlagBits);
EN(VkImageViewType);
EN(VkIndexType);
EN(VkLogicOp);
EN(VkMemoryPropertyFlagBits);
EN(VkObjectType);
EN(VkPipelineBindPoint);
EN(VkPipelineStageFlagBits);
EN(VkPolygonMode);
EN(VkPresentModeKHR);
EN(VkPrimitiveTopology);
EN(VkQueryResultFlagBits);
EN(VkQueryType);
EN(VkQueueFlagBits);
EN(VkResult);
EN(VkSampleCountFlagBits);
EN(VkSamplerAddressMode);
EN(VkSamplerMipmapMode);
EN(VkSamplerYcbcrModelConversion);
EN(VkSamplerYcbcrRange);
EN(VkShaderStageFlagBits);
EN(VkSharingMode);
EN(VkStencilOp);
EN(VkStructureType);
EN(VkSubpassContents);
EN(VkVertexInputRate);
...
ST(VkComputePipelineCreateInfo);
ST(VkDebugUtilsObjectNameInfoEXT);
...
ST(VkOffset2D);
ST(VkPhysicalDevice);
ST(VkPhysicalDeviceDescriptorIndexingProperties);
ST(VkPhysicalDeviceExtendedDynamicState3FeaturesEXT);
...
ST(VkQueryPool);
ST(VkQueryPoolCreateFlags);
ST(VkQueryPoolCreateInfo);
ST(VkQueryResultFlags);
ST(VkQueue);
